...
OPTS="--rename=pyyaml,python-yaml --rename=pyusb,python-usb --rename=websockify,websockify 
      ...
      --rename=beautifulsoup4,python-bs4 --rename=compose,python-docker-compose --rename=ipython,ipython"
...
if [ ! -e "/usr/bin/py2deb" ]; then
    sudo bash -c "echo deb file:///var/www/html/deb/trusty/solver_python/ / > /etc/apt/sources.list.d/py2deb.list"
    sudo apt-get update
